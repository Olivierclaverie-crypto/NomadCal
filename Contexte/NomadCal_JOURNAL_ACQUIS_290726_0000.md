...
- **RESTE : LOT B** (migration des notes orphelines vers `href`) puis **LOT A** (dédoublonnage iCloud, irréversible, backup frais). C répare le FUTUR (plus de bascule) ; B rattrape le PASSÉ (notes actuelles) ; A nettoie les doublons. *(Épilogue 19/07 soir : B fait par restauration, A rendu SANS OBJET par la table rase — voir entrée « CHANTIER COMPLET » ci-dessus.)*
...
- **DOUBLON D'AFFICHAGE POST-MODIF (non bloquant, chantier séparé).** Après édition, NC affiche transitoirement 2 versions de l'occurrence jusqu'à un refresh manuel → **données saines** (brut propre), pur affichage. Idée capitaine : refresh-auto après modif/création → chantier dédié (lot R1).
...
- **Bug C** — event à cheval sur minuit (`App.jsx:643-644` sans clip par jour). Pur affichage. Reste (lot R1).
...
- **DÉCISION BRIQUE 2 : `UNTIL` (date de fin), PAS `COUNT`** (rythme terrain = fin pensée en saison commerciale → toujours une date). *(COUNT reste au programme du compositeur complet — lot R2.)*
...
- Commits/PR : `ac0a025` (α scopé) · `4c2d5cc` (#31) · `e1e7763` (tunneling) · `70e595e` (PR-a) · `349ade1` (PR-b) · `ca19691` (#35 fix A) · #36 (enqueue offline) · #37 (EventForm) · #38 (fix B) · #39 (LOT C périodes, identité figée) · **#40 (C-bis, 8ᵉ site, mergé 19-07)**.
...
- **Bruts périodes (19/07) :** PROPFIND `nomadcal-oc` = 10 ressources (5 paires) AVANT table rase ; 5 ressources propres APRÈS (PRODID //NomadCal//FR). Export macOS `NomadCal_OC.ics` = 11 VEVENT (triplon Juil–Sept = cache client). Ressource-témoin C1 : `nomadcal-rapport-1782035089446@nomadcal` (etag `mrg6ps2h`→`mrg6ps2i` après note test).
- Backups locaux : export natif Settings 6 clés (16/07 12:49 + 19/07 20h56, 11 notes) + backup corrigé restauré (href complet). ⚠️ ne couvrent pas `nb_periods_cache`.
