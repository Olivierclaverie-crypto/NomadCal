...
*Document VIVANT — l'établi : SEULEMENT le chantier actif, les vérifs ouvertes, la reprise. Le déjà-fait scellé vit dans le JOURNAL/ACQUIS. Dernière MAJ : 15/07/2026 (**FIX BUG B MERGÉ #38** — fantôme mort, prouvé au brut zéro perte ; A+B corrigés ; prochaine étape = **bug C** ou sélecteur d'étendue).*
...
Le fix retire le **fantôme**. **Mais supprimer une occurrence *normale* efface encore la série** (toutes héritent `href = calflow-<ts>.ics`). C'est la **suppression d'occurrence** → chantier distinct : delete d'occurrence via `EXDATE` (ou exception `CANCELLED`), à coupler au **sélecteur d'étendue**. Hors périmètre B.

## 🎯 CHANTIER ACTIF = à choisir (capitaine)
Ordre recommandé : **Bug C** (le plus sûr) → puis **sélecteur d'étendue + delete d'occurrence** → puis paliers récurrence.
- **Bug C — event à cheval sur minuit mal affiché** (`App.jsx:643-644`, rendu sans clip par jour → recolle `startTime` sur J+1). **Pur affichage, réversible, aucune sacrée.** Correctif = clip par jour. Candidat idéal (faible risque). Brief EXÉCUTER à rédiger.

## 🩹 CHANTIERS SÉPARÉS (une variable chacun)
- **Sélecteur d'étendue (édition ET suppression : « celui-là / suivants / toute la série ») + delete d'occurrence (`EXDATE`).** N'existe pas dans NC. Ferme le dernier piège suppression (voir bloc ci-dessus). UI + écriture.
...
- **Bug B : ALLÉGÉE (fix #38).** Le fantôme n'existe plus. **Reste :** ne pas supprimer une **occurrence isolée** d'un récurrent (efface la série via href hérité) tant que le chantier delete-occurrence n'est pas fait. Si besoin : supprimer la série entière depuis NC + recréer.
...
## 🔭 BUGS RÉCURRENCE
- **A ✅ corrigé** (exception RFC + offline). **B ✅ corrigé** (#38, fantôme mort). **C — reste** (affichage minuit, chantier actif candidat).

## 📋 RESTE À FAIRE POUR V1 (ordre)
1. ~~Couche 2~~ · ~~bug A~~ · ~~récurrence-édition PR1+2~~ · ~~EventForm A1~~ · ~~verdict + fix B~~ → **FAITS.**
2. **Bug C** (sûr, chantier actif candidat).
3. **Sélecteur d'étendue + delete d'occurrence** (`EXDATE`).
...
3. **Chantier actif candidat = bug C** (affichage minuit, `App.jsx:643`, clip par jour — sûr, réversible). Rédiger le brief EXÉCUTER → cousin OK go → go Olivier → PR. *(Ou basculer sur sélecteur d'étendue / delete d'occurrence si tu préfères fermer le dernier piège suppression d'abord.)*
