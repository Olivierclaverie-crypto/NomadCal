# ROADMAP — NOMADCAL
*Moteur de PLANIFICATION : la source unique du « quoi, dans quel ordre, en combien de PR ». Organisée en LOTS BATCHÉS selon la doctrine de batch (→ Instructions) : batch autorisé hors zone sacrée, une variable à la fois STRICT près des sacrées. Chaque lot précise son risque, ses briefs, et si un BRAINSTORM est requis avant. L'ordre est une SUGGESTION — le capitaine réarbitre à chaque ouverture de lot. Dernière MAJ : 04-08-26 16h30 (lot T instruit de bout en bout → se solde en **1 seule PR** ; brainstorm « réordonner vers Neon » ouvert).*

---

## 🧭 VUE D'ENSEMBLE (ordre suggéré)
**~~R1~~ CLOS → T (PR-A, en cours) → E → R2 → R3 → R4**, avec **R5 et H en fillers** (carburant bas : c'est de la parole/des données, pas du code), **K juste avant Neon**, puis **pivot Neon → phase DESK**.

⚠️ **ARBITRAGE OUVERT (04-08) — « RÉORDONNER VERS NEON ».** Les congés (jusqu'au 01/09) referment la fenêtre de risque terrain. Deux lectures à ne pas confondre :
...
- **Non tranché — brainstorm dédié à ouvrir.**
...
| **T** | **Audit temporel 360°** → se solde en **1 PR (PR-A)** | **NON (zéro fautif en sacrée)** | ~~INVESTIGUER~~ ✅ → ~~CONSEILLER~~ ✅ → ~~décision~~ ✅ → **EXÉCUTER** | ✅ fait |
...
## 📅 JALONS (recalés 04-08-26 après-midi)
- **J1 — CONSOMMÉ (03/04-08)** : lot **N** + déménagement doc + **lot R1 entier** (4 PR en prod) + **lot T instruit de bout en bout** (INVESTIGUER, CONSEILLER, 2 décisions gravées, inventaire terrain, CERTIFIER parseur).
- **J2 — Congés (longues sessions)** : **PR-A** (lot T) → **E + R2** (EventForm, éventuellement fusionnés) · **R5 + H en fillers** (soirs carburant bas) · **R3** si carburant devant soi. **Brainstorms à ouvrir : méthode · EventForm · réordonner vers Neon.**
...
- **Item 2** (doublon transitoire) → **#41**. Cause = clé de déduplication mal formée.
- **Item 3** (bande all-day sur 2 jours) → **#43**. 3 sites + prédicat partagé + repli. Cas **nominal**.
- **Item 1** (event à cheval sur minuit) → **#44 (cause racine fuseau) + #45 (segments jour)**.
- **Découvertes adjacentes** → lots **E** et **T**.

---

## LOT T — AUDIT TEMPOREL 360° (étape 3/3 — SE SOLDE EN 1 PR)
...
**Résultat de l'audit (43 sites — A:31 · B:8 · C:2 · corrigés:2) : ⭐ ZÉRO fautif en zone sacrée.**
➡️ **Le rapport intégral + la certification parseur + l'inventaire terrain vivent dans l'ÉTAT** jusqu'au scellement de la PR-A.

### ✅ CE QUI EST TRANCHÉ (04-08)
1. **FUSEAU → option (a) : NomadCal vit en heure de Paris, par décision.** Douteux C1 **fermé**, zéro ligne de code, `pushEvent` jamais touchée, totalement réversible. Doctrine portée aux **INSTRUCTIONS**.
2. **Famille 1a → périmètre « Z SEUL »**, TZID exotique en dette.
3. **INVENTAIRE TERRAIN** (4 bruts, 4 producteurs) : **zéro `Z`, zéro TZID exotique, zéro `UNTIL` d'event.** Google **et** Outlook écrivent en `TZID=Europe/Paris` — les deux producteurs cibles de la famille 1a. **Population exposée nulle, mesurée.**
4. **CERTIFIER parseur : 3 OUI** — VTIMEZONE inerte · TZID lu en heure murale (prouvé au code) · TRIGGER inerte.
5. **DÉCISION DE SORTIE : PR-A SEULE.** Familles 1a + 1b → **dette tracée** (file d'attente).

### ▶️ PR-A — FAMILLE 2 (`new Date("YYYY-MM-DD")` = minuit UTC)
- **5 sites**, 2 fichiers (`NomadBook.jsx` + `App.jsx:771`), ≈ **+8/−8**. **Hors zone sacrée.**
- **⚠️ Ce n'est PAS de l'affichage** (écart corrigé par le CONSEILLER) : `getPeriodStatus` → `currentPeriod` → `periodId` d'une note **et** `syncNoteCount` qui **ÉCRIT sur iCloud**. Une note saisie 00h–02h le 1er jour d'une période part sur la **période précédente**. Défaut de **donnée**.
- **⭐ Carte des 3 idiomes** (dans le brief) : affichage → pivot midi · comparaison d'instant → minuit **local** · « aujourd'hui » → `todayISO` existant.
- **Aucun identifiant ne bouge** (les `href` viennent des chaînes `endISO`, jamais d'un `Date`).
- **Cas de test fixé** : « FORMULAIRE VEILLE », **zéro écriture**, horloge ~00h30. Détail → ÉTAT.

### 📌 SORTIE ATTENDUE DU LOT
La **règle unique** portée aux INSTRUCTIONS (§ DOCTRINE TEMPORELLE) — **déjà rédigée**, enrichie le 04-08 de la doctrine fuseau (règle 7). À vérifier au scellement, pas à produire.
...
- ⚠️ **Attendre le lot T** : ce lot manipule intensivement des dates.
- ⚠️ **Ajout 04-08** : la dette **famille 1a `EXDATE`** (`caldav.js:152`, strip du `Z`) vit dans le territoire exact de ce lot. Si R3 s'ouvre, **la reprendre à ce moment-là** plutôt qu'isolément.
...
**Issues du lot T (04-08) — détail et preuves dans l'ÉTAT :**
...
- **Brèche `DESCRIPTION` de VALARM** (`caldav.js:109`/`:122`/`:145`) : la `DESCRIPTION` d'une alarme peut atterrir dans les **notes de l'event** si l'event n'a pas la sienne. Chemin réel au code, cas non observé. **Hors lot T.**
...
- **⭐ RÉORDONNER LA ROADMAP VERS NEON** (04-08) — voir § VUE D'ENSEMBLE. Non tranché.
- **Brainstorm MÉTHODE** (03-08) : fixer le cas de test avant le brief (piste 1, déjà portée aux Instructions) · scripts de test automatisés (piste 2 — réaliste sur la **logique**, pas sur la fenêtre : ni le timonier ni le cousin n'ont accès à iCloud) · audit ciblé (piste 3).
